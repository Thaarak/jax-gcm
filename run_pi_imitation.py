...
def parse_args():
    ...
    p.add_argument("--f0-range", type=float, nargs=2, default=(-0.25, 0.05),
                   help="Realized-dSST sampling range for feature 0.")
...
def sample_features(rng, batch, feature_dim, f0_range):
    """Synthetic fc13 feature batch covering the PI-relevant subspace.

    The target depends only on features 0 (realized dSST) and 10 (time
    fraction); every other feature is sampled over a generous superset of
    its real rollout range so the distilled net is INVARIANT to it across
    the states it will actually see (not just near zero).
    """
    ...
    if feature_dim > 11:
        f[:, 11] = F11_ANCHOR + rng.uniform(-1.5, 1.5, batch)
    if feature_dim > 12:
        f[:, 12] = F12_ANCHOR + rng.uniform(-2.0, 2.0, batch)
    return f
...
def distill(pattern, args, log_every=500):
    """Train the fc13 MLP to reproduce the PI law; returns (params, report).
    ...
    """
    feature_dim = get_coupled_feature_dim(FC13)
    policy = MCBPolicyMLP(
    ...
    big = sample_features(stat_rng, 50_000, feature_dim, args.f0_range)
    mu = big.mean(axis=0)
    ...
    for i in range(args.steps):
        feats = jnp.asarray(
            sample_features(rng, args.batch, feature_dim, args.f0_range),
            dtype=jnp.float32)
    ...
    feats = jnp.asarray(
        sample_features(grid_rng, 2048, feature_dim, args.f0_range),
        dtype=jnp.float32)
...
def main():
    ...
    print("=" * 72)
    print(f"PI-IMITATION DISTILLATION (seed {args.seed})")
    print("=" * 72)
