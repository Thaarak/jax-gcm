#!/usr/bin/env bash
# Stage 5 Option A pipeline runner (diya GB10). Launched inside tmux.
# Seasonal bracketing: train {0,45,90,135,180,225}, held-out {70,200} — the
# held-out ICs sit strictly INSIDE the training-day hull, converting held-out
# evaluation from extrapolation into interpolation.
# Stops vLLM before JAX, ALWAYS restarts it at the end (trap), even on failure.
set -uo pipefail

cd ~/workspace/jax-gcm
PY=~/mcb-env/bin/python
S5=mcb_experiments/stage5_optionA
mkdir -p $S5
...
echo "[pipeline] ===== IC GENERATION (bracketed schedule) ====="
$PY run_stage5_generate_ics.py --realistic-terrain \
  --train-days 0,45,90,135,180,225 --heldout-days 70,200 \
