#!/usr/bin/env python
"""Stage 4 IC generation: spin-up snapshots + paired baselines (run once).

Runs the no-MCB coupled aquaplanet forward from the standard start date,
saving full coupled carries at spin-up days {0, k, 2k, ...} (k =
--spinup-interval). The first --num-train snapshots are the training ICs;
the next --num-heldout are held out for generalization testing. Each IC gets
its own paired --horizon-day no-MCB baseline trajectory (computed from that
exact carry, so paired anomalies cancel natural drift exactly).

Because insolation follows the carry's sim_time (not the step index), ICs at
different spin-up days sample different seasons as well as different weather.

Usage:
    python run_stage4_generate_ics.py --output-dir mcb_experiments/stage4/ics
"""
...
def parse_args():
    parser = argparse.ArgumentParser(description="Stage 4 IC generation")
...
def main():
    ...
    carry["atm"]["derived"]["mcb_perturbation"] = jax.numpy.zeros_like(existing)
